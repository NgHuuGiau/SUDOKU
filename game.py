...
class Game:
    ...
    def handle_events(self) -> bool:
        for event in pygame.event.get():
            ...
            if self.state.game_over and (event.type == pygame.KEYDOWN or event.type == pygame.MOUSEBUTTONDOWN):
                self.running = False
                return True
            ...
            if event.type == pygame.ACTIVEEVENT:
                ...
                if event.gain == 1 and not self.state.paused:
                    self.state.paused_time += pygame.time.get_ticks() - self.state.last_active_time

            if event.type == pygame.MOUSEBUTTONDOWN:
                self._handle_mouse(event.pos)

            if event.type == pygame.KEYDOWN and not self.state.game_over and not self.state.paused:
    ...
    def _handle_mouse(self, pos: Tuple[int, int]) -> None:
        ...
        if self.state.paused:
            resume_btn = pygame.Rect(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 20, 200, 50)
    ...
    def render(self) -> None:
        ...
        if self.state.selected and not self.state.paused:
            ...
            rect = pygame.Rect(c * CELL_SIZE + PADDING,
                              r * CELL_SIZE + TOP_MARGIN, CELL_SIZE, CELL_SIZE)
        ...
        if self.state.selected and not self.state.paused:
            rect = pygame.Rect(c_sel * CELL_SIZE + PADDING,
        ...
        if self.state.game_over:
            ...
            sub_text = self.small_font.render(_("nhan_phim_bat_ky"), True, Colors.WHITE)
        ...
        if self.state.paused:
            ...
            self.screen.blit(pause_text, pause_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 40)))


            resume_btn_rect = pygame.Rect(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 20, 200, 50)
            draw_button(self.screen, resume_btn_rect, _("tiep_tuc"), mouse_pos, self.small_font, 'warning')
    ...
    def _draw_controls(self, mouse_pos, start_y):
        ...
        for text, action, color_scheme in buttons_list:
            rect = draw_button(self.screen, pygame.Rect(PADDING + BOARD_SIZE + PANEL_OFFSET + 15,
                                          y, btn_w, btn_h),
        ...
        clear_rect = pygame.Rect(PADDING + BOARD_SIZE + PANEL_OFFSET + 15 +
                                  (num_btn_w + num_pad) * 1,
